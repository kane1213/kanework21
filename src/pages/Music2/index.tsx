...
export default (props: any) => {
  ...
  function musicBoxStart() {
    gaspRef.current = gsap.to(notesRef.current, {
      ...
      onComplete() {
        notesRef.current.y = CANVAS_HEIGHT * 0.5 + 5 - parseInt(START_HEIGHT)
        finishNotes.current = []
  ...
  function playMusicEvent(event: any) {
    console.log('start')
    copyToClipboard(
      `${MUSICBOX_TITLE} - 【懷舊】${MUSICBOX_SUBTITLE} #musicbox #音樂盒 引用的影片 https://www.youtube.com/watch?v=${ytId}`
    )
  ...
  function handleVideo() {
    ...
    const videoId = ytId
    player.current = YouTubePlayer('video-player', {
      videoId,
      width,
      height: 255,
      rates: 2,
  ...
  return (
    <div className={SHOWARROW ? '' : 'cursor-none'}>
      ...
      >
        <Container x={(CANVAS_WIDTH - 1240) * 0.5} y={25}>
          <Sprite
            width={1240}
            height={681}
